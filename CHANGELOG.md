...
- **v3.12.3** (2026-09-18): a run marked `execution.delegated`, driven by an external orchestrator that owns the workspace, may request `access: full` without a user-level trust record. Runs an operator starts at a surface are unchanged: they still require the one-time grant and still receive the same typed 403 refusal with its retry remedy, and the effective profile of an admitted delegated run is still recorded as `full` rather than downgraded. Any client that can reach the daemon control API or the host MCP server can set this marker, and such a client already holds the daemon token that could write the trust file itself; the macOS app and belt sub-runs cannot set it. The Claudexor.app compatibility floor stays at 3.11.0 (no new wire contract the app must decode).
